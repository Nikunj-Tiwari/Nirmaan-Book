  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-2">
        <h2 className="text-[var(--text-secondary)] uppercase text-[10px] font-bold tracking-widest">Dimensions</h2>
        <div className="bg-[var(--bg-secondary)] p-4 rounded-lg border border-[var(--border)] flex flex-col gap-2">
          <div className="flex justify-between items-end">
            ...
            <span className="text-xl font-mono text-[var(--accent)]">{calculateUsedWidth(state.modules)}<small className="text-[10px] ml-1">mm</small></span>
          </div>
          <div className="flex justify-between items-end">
            <span className="text-xs text-[var(--text-secondary)]">Remaining:</span>
            <span className={`text-xl font-mono ${remaining < 600 ? 'text-[var(--danger)]' : 'text-emerald-400'}`}>{remaining}<small className="text-[10px] ml-1">mm</small></span>
          </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-2 px-2">
          <div className="w-4 h-4 rounded-full border border-white/10 shadow-lg" style={{ backgroundColor: state.material?.color }} />
          <span className="text-[10px] text-white/40 uppercase font-mono tracking-tight">Active Style: {state.material?.color}</span>
        </div>
      </div>
          ...
          {modules.map((mod) => (
            ...
            >
              <div className="flex justify-between items-center w-full">
                <span className="text-sm font-medium group-hover:text-[var(--accent)] text-[var(--text-primary)]">{mod.name}</span>
                <span className="text-[10px] bg-[var(--bg-primary)] px-1.5 py-0.5 rounded text-[var(--text-secondary)]">{mod.width}mm</span>
              </div>
              <span className="text-xs text-[var(--text-secondary)]">₹ {mod.basePrice.toLocaleString()}</span>
            </button>
          ))}
        </div>
      </div>
