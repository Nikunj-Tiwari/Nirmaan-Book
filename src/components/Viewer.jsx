...
export const Viewer = () => {
  ...
  return (
    <div className="flex-1 bg-[var(--bg-secondary)] rounded-2xl border border-[var(--border)] flex items-center justify-center relative overflow-hidden group min-h-[450px] shadow-lg">
      ...
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: 'linear-gradient(var(--border) 1px, transparent 1px), linear-gradient(90deg, var(--border) 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>
      ...
      <div className="text-center z-10 flex flex-col items-center gap-8 p-12">
        ...
        <div className="flex flex-col gap-3">
           <h4 className="text-2xl font-black text-[var(--text-primary)] uppercase tracking-tight">3D Visualization</h4>
        ...
        <div className="grid grid-cols-2 gap-4 w-full max-w-sm mt-4">
            <div className="bg-[var(--bg-primary)] p-5 rounded-xl border border-[var(--border)] flex flex-col items-center shadow-sm">
               <span className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-[2px] mb-2 opacity-50">State Payload</span>
               <span className="text-sm font-black text-[var(--accent)]">{totalModules} UNITS</span>
            </div>
            <div className="bg-[var(--bg-primary)] p-5 rounded-xl border border-[var(--border)] flex flex-col items-center shadow-sm">
               <span className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-[2px] mb-2 opacity-50">Texture Atlas</span>
               <span className="text-sm font-black text-[var(--text-primary)]">{material?.name.toUpperCase()}</span>
            </div>
        </div>
      </div>
    </div>
