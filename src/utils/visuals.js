...
export const getElevationLayout = (config, cw, ch) => {
  ...
  primitives.push({ type: 'rect', x: ox - 100, y: oy - 100, w: rw + 200, h: rh + 200, fill: '#0f1115', stroke: 'rgba(255,255,255,0.1)', lineWidth: 1 });
  ...
  modulesList.forEach((mod, i) => {
    ...
    if (layout.shelves > 0) {
      for (let j = 1; j <= layout.shelves; j++) {
        ...
        primitives.push({ type: 'line', x1: mx + 2, y1: sy, x2: mx + mw - 2, y2: sy, stroke: 'rgba(255,255,255,0.1)', lineWidth: 1.5 });
      }
    }
    ...
    if (layout.hang > 0) {
      for (let j = 0; j < layout.hang; j++) {
        ...
        primitives.push({ type: 'line', x1: mx + 10, y1: hy, x2: mx + mw - 10, y2: hy, stroke: 'rgba(255,255,255,0.4)', lineWidth: 3 });
      }
    }
    ...
    if (layout.drawers > 0) {
      ...
      for (let j = 0; j < layout.drawers; j++) {
        ...
        primitives.push({ type: 'rect', x: mx + 4, y: dy + 2, w: mw - 8, h: drawerH - 4, fill: 'rgba(79, 140, 255, 0.05)', stroke: 'rgba(255,255,255,0.1)', lineWidth: 1 });
        primitives.push({ type: 'rect', x: mx + mw/2 - 8, y: dy + drawerH/2 - 1, w: 16, h: 2, fill: 'rgba(255,255,255,0.3)' }); // Handle
      }
    }
  ...
  primitives.push({ type: 'text', x: ox + rw/2, y: oy + rh + 60, text: `${totalW} MM TOTAL WIDTH`, font: '900 12px Inter,sans-serif', align: 'center', fill: '#4f8cff' });
